...
export default function CheckoutPage({ params: { lang } }: { params: { lang: Locale } }) {
  ...
  const mapErrorMessage = (rawError: string): string => {
    if (rawError.includes('OUT_OF_STOCK')) return lang === 'ru' ? 'Недостаточное количество товара на складе.' : 'Mahsulot omborda yetarli miqdorda mavjud emas.';
    if (rawError.includes('PRODUCT_UNAVAILABLE')) return lang === 'ru' ? 'Товар недоступен для заказа.' : 'Mahsulot hozirda sotuvda mavjud emas.';
    if (rawError.includes('INVALID_VARIANT')) return lang === 'ru' ? 'Выбранный вариант товара недоступен.' : 'Tanlangan variant mavjud emas.';
    return rawError;
  };

  const validateForm = () => {
    const errors: Record<string, string> = {};
    if (!customerName.trim() || customerName.trim().length < 2) errors.name = lang === 'ru' ? 'Введите имя' : 'Ismingizni kiriting';
    if (!validatePhone(customerPhone)) errors.phone = lang === 'ru' ? 'Введите корректный номер +998' : 'To‘g‘ri telefon kiriting +998 XX XXX XX XX';
    if (deliveryType === 'COURIER' && !address.trim()) errors.address = lang === 'ru' ? 'Введите адрес' : 'Manzilni kiriting';
    if (!agree) errors.agree = lang === 'ru' ? 'Примите условия' : 'Shartlarga rozilik bildiring';
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.order) {
        trackEvent('purchase', { transaction_id: data.order.orderNumber, value: finalTotal, currency: 'UZS', coupon: appliedCoupon?.code });
        clearCart();
              ...
              <label className="flex items-start gap-2.5 p-3 rounded-xl bg-gray-50 border border-gray-200 cursor-pointer">
                ...
                <span className="text-xs text-gray-600 leading-relaxed">
                  {lang === 'ru' ? (
                    ...
                    <>
                      <a href={`/${lang}/terms`} className="text-brand-red underline">Ommaviy oferta</a> va <a href={`/${lang}/privacy`} className="text-brand-red underline">maxfiylik siyosati</a>ga roziman
                    </>
                  )}
                </span>
              </label>
              ...
              <div className="space-y-3 text-sm">
                ...
                {appliedCoupon && (
                  <div className="flex justify-between text-emerald-700 font-bold p-2.5 rounded-xl bg-emerald-50 border border-emerald-200">
                    ...
                    <span>-{formatPrice(couponDiscount, lang)}</span>
                  </div>
                )}
