...
export const ProductCard: React.FC<ProductCardProps> = ({ product, lang, featured = false }) => {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-xl overflow-hidden flex flex-col justify-between h-full border border-gray-100">
        ...
        {/* Favorite Button (Button-in-Button style) */}
        <button
          onClick={toggleFavorite}
          className="absolute top-3 right-3 z-10 w-7 h-7 rounded-full bg-white/90 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-brand-red transition-all shadow-xs hover:scale-105"
          aria-label="Favorite"
        >
          <Heart className={`w-3.5 h-3.5 ${isLiked ? 'fill-brand-red text-brand-red' : ''}`} />
        </button>

        {/* Product Photography Container */}
        <Link
          href={`/${lang}/product/${product.slug}`}
          className="block relative aspect-square w-full bg-[#F8F9FA] overflow-hidden flex items-center justify-center p-3 border-b border-gray-100 group-hover:bg-[#F3F4F6] transition-colors"
        >
          {mainImage && !imgError ? (
            <Image
              src={mainImage}
        ...
        <div className="p-3.5 flex flex-col flex-1 justify-between gap-3 bg-white">
          ...
          {/* Pricing & CTA Button */}
          <div className="pt-1 flex flex-col gap-2.5 mt-auto border-t border-gray-100">
            <Price
              price={product.price}
            ...
            {product.hasVariants ? (
              ...
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 text-xs font-semibold py-1.5 pl-3 pr-1.5 rounded-lg transition-colors ${
                  added
